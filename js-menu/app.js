...
function displayMenuItems(menuItems) {
  // Loop trough Array use MAP method
...
function displayMenuBtns() {
  // accumulator = value
  // current = item
  const category = menu.reduce((accumulator, current) => {
    // check if value already in the item category
    if (!accumulator.includes(current.category)) {
      // if not yet in the category push the category into the string
      accumulator.push(current.category);
    }
    return accumulator;
  }, [ 'all' ]);
  // loop all the dynamics buttons that we just created
  const categoryBtns = category.map(category => {
    return `
      <button class="filter-btn" type="button" data-id="${category}">${category}</button>
    `
  }).join('');
  btnContainerEl.innerHTML = categoryBtns;
  const filterBtnsEl = btnContainerEl.querySelectorAll(".filter-btn");
  // filter items
